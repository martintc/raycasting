...
void castRay(float rayAngle, int stripId) {
  ...
  while (isInsideMap(nextHorzTouchX, nextHorzTouchY)) {
    ...
    if (mapHasWallAt(xToCheck, yToCheck)) {
      ...
      horzWallContent = getMapAt((int)floor(yToCheck / TILE_SIZE),
  ...
  while (isInsideMap(nextVertTouchX, nextVertTouchY)) {
    ...
    if (mapHasWallAt(xToCheck, yToCheck)) {
      ...
      vertWallContent = getMapAt((int)floor(yToCheck / TILE_SIZE),
				 (int)floor(xToCheck / TILE_SIZE));
...
void renderRays(void) {
  for (int i = 0; i < NUM_RAYS; i++) {
    drawLine(player.x * MINIMAP_SCALE_FACTOR, player.y * MINIMAP_SCALE_FACTOR,
